...
RUN /etc/init.d/postgresql start &&\
    psql --command "CREATE USER tp WITH SUPERUSER PASSWORD 'tp';" &&\
    createdb -O tp tp &&\
...
CMD service postgresql start &&  psql -h localhost -d tp -U tp -p 5432 -a -q -f ./db/db.sql && ./main